...
#include <fstream>
#include "rc4openssl/rc4_enc.c"
#include "rc4openssl/rc4_skey.c"
...
int rc4(int fd){
        ...
        passwordFile.open("/home/pk/Desktop/pw.txt");
	string password;
	getline(passwordFile,password);

	int fdlength = lseek(fd, 0, SEEK_END);

	unsigned char* fdbuffer;
	fdbuffer = (unsigned char*) calloc(fdlength, sizeof(char));

	pread(fd, fdbuffer, fdlength, 0);
	string fd_str(reinterpret_cast<char*>(fdbuffer), fdlength);
	char const* fd_cstr = fd_str.c_str();

	RC4_KEY pkey;
	int datalength = fd_str.size();
	unsigned char *buffer = (unsigned char*)malloc(datalength + 1);
	memset(buffer, 0, datalength + 1);

	RC4_set_key(&pkey, password.length(), (const unsigned char*)password.c_str());
	RC4(&pkey, datalength, (const unsigned char*)fd_cstr, buffer);

	string ret((char*)buffer, datalength);
	const char *outbuffer = ret.c_str();
	pwrite(fd, outbuffer, ret.length(), 0);
	ftruncate(fd, ret.length());
	return 0;
...
{
//First attempt to decrypt
/*size = 4096;
RC4_KEY pkey;
unsigned char encryptKey[9]="pemching";
RC4_set_key(&pkey,9,encryptKey);

ofstream test2;

test2.open("//home//pk//Desktop//testreadsize.txt", ofstream::out | ofstream::app);
test2 << "read size " << size << endl;
test2.close();*/
...
//RC4(&pkey, 4096, (const unsigned char *) bytes, (unsigned char *)bytes);
...
//RC4_KEY pkey;
//const unsigned char encryptKey[9]="pemching";
//RC4_set_key(&pkey, Key_Length, (const unsigned char*)Key);
/*
ofstream test;

test.open("//home//pk//Desktop//testwritesize.txt", ofstream::out | ofstream::app);
test << "Write size" << size << endl;
test.close();*/
...
//RC4(&pkey, size, (const unsigned char*) bytes, (unsigned char *)bytes);
